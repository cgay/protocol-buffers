...
define test test-parse-varint ()
  let frame = parse-frame(<varint-frame>, #(#x01, #x02, #x03));
  assert-equal(frame.varint-bytes, bv(#x01));

  let frame = parse-frame(<varint-frame>, #(#x81, #x02, #x03));
  assert-equal(frame.varint-bytes, bv(#x81, #x02));
end test;
